...
watch(executionLogs, (logs) => {
    ...
    if (statusLogs.length > 0) {
        ...
        if (lastStatusMsg.includes('SUCCEEDED')) jobStatus.value = { text: 'Succeeded', color: 'success', icon: 'mdi-check-circle-outline' };
        else if (lastStatusMsg.includes('FAILED')) jobStatus.value = { text: 'Failed', color: 'error', icon: 'mdi-alert-circle-outline' };
        else if (lastStatusMsg.includes('STOPPED')) jobStatus.value = { text: 'Stopped', color: 'warning', icon: 'mdi-stop-circle-outline' };
        isRunning.value = false;
        isSimulating.value = false;
        isStopping.value = false;
        currentJobId.value = null;
