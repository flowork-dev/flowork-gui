...
// File NAME : C:\FLOWORK\flowork-gui\template\web\src\views\Designer.vue total lines 533 
//#######################################################################
          ...
          <div class="hacker-log-stream">
              <transition-group name="log-slide">
                  <div v-for="log in visibleLogs" :key="log.id" class="stream-line">
                      ...
                      <span class="stream-source"> &lt;{{ log.source }}&gt;</span>
                  </div>
              </transition-group>
          </div>
...
/* [HACKER MODE] GHOST STREAM STYLING - REVISED */
.hacker-log-stream {
    position: absolute;
    bottom: 0; /* Mulai dari bawah */
    right: 0; /* Rata kanan */
    width: 450px; /* Lebar log area */
    height: 50%; /* Setengah tinggi canvas */
    padding: 20px 30px;

    display: flex;
    flex-direction: column;
    justify-content: flex-end; /* Stack dari bawah */
    align-items: flex-end; /* Text Align Right */

    pointer-events: none; /* Klik tembus */
    z-index: 10;

    /* Masking: Fade Out Halus di Bagian Atas (biar ga kepotong kasar) */
...
.stream-line {
    ...
    font-size: 13px;
    line-height: 1.4;
    /* Shadow biar kebaca jelas di background apapun */
...
.log-slide-enter-from {
  opacity: 0;
  transform: translateY(20px); /* Muncul naik dari bawah */
}
.log-slide-leave-to {
  opacity: 0;
  /* Hilang pelan-pelan, ga perlu geser biar ga aneh */
}
