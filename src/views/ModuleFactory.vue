...
// File NAME : src/views/ModuleFactory.vue
//#######################################################################
  ...
  <div class="module-factory-layout h-100 d-flex flex-column bg-black">
    ...
    <div class="forge-header px-4 py-2 d-flex align-center border-bottom-subtle flex-shrink-0 bg-glass">
        ...
        <div class="d-flex flex-column">
            <span class="text-subtitle-2 font-weight-bold orbitron-font text-amber-darken-1">
                COMPONENT FORGE <span class="text-grey-darken-2 text-caption">v1.2</span>
            </span>
    ...
    <div v-else class="workspace-container flex-grow-1 d-flex overflow-hidden">
        ...
        <div class="editor-main flex-grow-1 d-flex flex-column bg-black relative">
            <div class="editor-tabs d-flex bg-black border-bottom-subtle">
...
// Metadata for the component (Visual & ID)
const meta = reactive({
    name: 'New Component',
...
// Schema for inputs (properties) and outputs
const schema = reactive({
    inputs: [], // Mapped to Properties
    outputs: [] // Mapped to Output Ports
});

// Manifest Object (Source of Truth for the Engine)
// This mirrors ManifestModel.py structure
...
// Computed Manifest JSON String for the Viewer
...
// --------------------------------------------------------------------------
// CORE LOGIC: Initialize Forge
// --------------------------------------------------------------------------
function initForge(type) {
    ...
    // Generate Boilerplate Code based on Type (Context Aware)
...
// --------------------------------------------------------------------------
// TEMPLATE ENGINE: Context Aware Python Boilerplate
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// SYNC ENGINE: Manifest <-> UI Binding
// --------------------------------------------------------------------------
function syncManifest() {
    ...
    // Sync Inputs (Properties)
    // Mapping UI inputs -> Manifest properties
    ...
    // Sync Outputs (Ports & Schema)
    // Mapping UI outputs -> Manifest output_ports
    ...
    // Also update output schema definition
...
function updateIcon(icon) {
    meta.icon = icon;
    // Manifest model doesn't strictly have icon path yet in Python model shown,
    // but usually it's in display_properties or separate asset.
    // For now we keep it in meta for UI.
}
...
function injectCode(snippet) {
    // Simple injection logic (append to end or cursor position if we had reference)
    // For MVP appending relevant snippets
    let code = "";
...
async function saveComponent() {
    ...
    // Simulate Packaging
    console.log("Packaging Component...");
    console.log("Manifest:", manifest);
    console.log("Code:", codeContent.value);
    console.log("Requirements:", requirementsContent.value);

    // TODO: Connect to backend API /api/v1/components/install
    // payload = { manifest, code: codeContent.value, requirements: requirementsContent.value }

    setTimeout(() => {
        isSaving.value = false;
        uiStore.showNotification({ text: "Build Saved to Local Engine.", color: "success" });
    }, 1000);
}

function testRun() {
    uiStore.showNotification({ text: "Sending to Sandbox...", color: "info" });
    // Future: WebSocket send to engine for dry-run
}
</script>
