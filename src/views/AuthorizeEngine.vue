...
import { useAuthStore } from '@/store/auth'; // (PENAMBAHAN KODE)
import { apiSelectEngineForAuth } from '@/api';

const route = useRoute();
const router = useRouter(); // (PENAMBAHAN KODE)
const engineStore = useEngineStore();
const authStore = useAuthStore(); // (PENAMBAHAN KODE)

const step = ref('loading');
const errorMessage = ref('');
const requestId = ref(null);

// (PENAMBAHAN KODE) Fungsi baru untuk redirect ke halaman login
function goToLogin() {
...
onMounted(async () => {
    ...
    // (PERBAIKAN KUNCI) Cek status login di sini
