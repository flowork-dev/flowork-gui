...
// File NAME : C:\FLOWORK\flowork-gui\template\web\src\views\Dashboard.vue total lines 404 
//#######################################################################
      ...
      <v-row v-else dense>
        ...
        <v-col cols="12" md="3" class="d-flex flex-column" style="gap: 16px;">
              ...
              <div v-else>
                <div v-for="engine in engines" :key="engine.id" class="engine-status-item">
                  <div class="d-flex align-center">
                    ...
                    <div>
                      ...
                      <div class="text-caption" :class="engine.status === 'online' ? 'text-green-accent-4' : 'text-grey-darken-1'">
                        {{ engine.status ? engine.status.toUpperCase() : 'UNKNOWN' }}
                        <span v-if="engine.vitals?.kernel_version"> - v{{ engine.vitals.kernel_version }}</span>
                      </div>
                    </div>
                    <v-spacer></v-spacer>
                  ...
                  <div v-if="engine.status === 'online' && engine.vitals" class="vitals-grid mt-2">
                    <div class="text-caption">CPU: {{ (engine.vitals.cpu_percent || 0).toFixed(1) }}%</div>
                    <v-progress-linear :model-value="engine.vitals.cpu_percent || 0" color="cyan" height="6" rounded></v-progress-linear>
                    <div class="text-caption">RAM: {{ (engine.vitals.ram_percent || 0).toFixed(1) }}%</div>
                    <v-progress-linear :model-value="engine.vitals.ram_percent || 0" color="purple-accent-2" height="6" rounded></v-progress-linear>
                  </div>
                </div>
              </div>
            </v-card-text>
          ...
          <v-card class="dashboard-card" variant="flat">
             ...
             <div v-if="(!summary.top_failing_presets || summary.top_failing_presets.length === 0) && (!summary.top_slowest_presets || summary.top_slowest_presets.length === 0)" class="text-center text-grey py-4">
             ...
             <v-list v-else dense density="compact" bg-color="transparent" class="pt-0">
                  <template v-if="summary.top_failing_presets && summary.top_failing_presets.length > 0">
                  ...
                  <template v-if="summary.top_slowest_presets && summary.top_slowest_presets.length > 0">
...
watch(selectedEngineId, (newId) => {
    if (newId) {
        console.log(`[Dashboard] Engine switched to ${newId}. Reloading data...`);
...
onMounted(async () => {
  if (authStore.isAuthenticated) {
      console.log("[Dashboard] Page mounted. Forcing engine status check...");
