          ...
          <v-col cols="12" md="8">
            ...
            <v-card v-else-if="article" class="article-card-content">
              ...
              <div class="article-meta">
                <span>By:
                  <router-link :to="`/author/${article.author_username}`" class="article-link">
                    <strong id="article-author">{{ displayAuthorName }}</strong>
                  </router-link>
                </span>
                <span>Category:
                  <router-link :to="`/category/${article.category}`" class="article-link">
...
function updateSEOTags(articleData) {
  document.title = `${articleData.title} | Flowork`; // English Hardcode
  let metaDesc = document.querySelector('meta[name="description"]'); // English Hardcode
  if (!metaDesc) {
    metaDesc = document.createElement('meta');
    metaDesc.setAttribute('name', 'description'); // English Hardcode
    document.head.appendChild(metaDesc);
  }
  metaDesc.setAttribute('content', articleData.meta_description || articleData.title);
  if (articleData.keywords) {
    let metaKeywords = document.querySelector('meta[name="keywords"]'); // English Hardcode
    if (!metaKeywords) {
        metaKeywords = document.createElement('meta');
        metaKeywords.setAttribute('name', 'keywords'); // English Hardcode
...
function updateStructuredData(articleData) {
  const oldScript = document.getElementById('article-structured-data'); // English Hardcode
  if (oldScript) {
    oldScript.remove();
  }
  const lang = articleData.language || 'en'; // English Hardcode
  const fullUrl = `https://flowork.cloud/p-${articleData.slug}-${lang}.html`; // English Hardcode
  const schema = {
    "@context": "https://schema.org", // English Hardcode
    "@type": "BlogPosting", // English Hardcode
    "mainEntityOfPage": { // English Hardcode
      "@type": "WebPage", // English Hardcode
      "@id": fullUrl // English Hardcode
    },
    "headline": articleData.title, // English Hardcode
    "description": articleData.meta_description || articleData.title, // English Hardcode
    "author": { // English Hardcode
      "@type": "Person", // English Hardcode
      "name": articleData.author_username || "Flowork Team" // English Hardcode
    },
    "publisher": { // English Hardcode
      "@type": "Organization", // English Hardcode
      "name": "Flowork", // English Hardcode
      "logo": { // English Hardcode
        "@type": "ImageObject", // English Hardcode
        "url": "https://flowork.cloud/favicon.svg" // English Hardcode
      }
    },
    "datePublished": new Date(articleData.publish_at || articleData.created_at).toISOString(), // English Hardcode
    "dateModified": new Date(articleData.updated_at).toISOString() // English Hardcode
  };
  const script = document.createElement('script');
  script.type = 'application/ld+json'; // English Hardcode
  script.id = 'article-structured-data'; // English Hardcode
...
async function fetchArticle() {
  ...
  if (!slug || !lang) {
      error.value = "Invalid URL structure."; // English Hardcode
  ...
  if (result.error) {
    ...
    document.title = "Article Not Found | Flowork"; // English Hardcode
  } else {
...
const displayAuthorName = computed(() => {
  ...
  if (!username) {
    return 'Flowork Team'; // English Hardcode
  }
