...
async function handleResetToken(engine) {
    const confirmed = await uiStore.showConfirmation({
        ...
        text: `Are you sure you want to reset the token for "${engine.name}"? You must update the .env file on that engine.`,
        color: 'warning',
        confirmText: 'Reset'
    });
    if (confirmed) {
        ...
        try {
            ...
            uiStore.showTokenDialog({
                ...
                text: 'IMPORTANT: Copy these details and update your engine\'s C:\\FLOWORK\\.env file. The token is only shown once!',
                items: [
            ...
            const errorMsg = e.error || e.message || 'Failed to reset token.';
            console.error('[MyEngines] Error resetting token:', errorMsg);
...
async function confirmRenameEngine() {
    if (!engineToRename.value || !renameEngineName.value || !renameEngineName.value.trim()) {
        uiStore.showNotification({ text: 'Please enter a valid new name.', color: 'warning' });
        return;
    }
    let localIsLoading = ref(false);
    try {
        ...
        uiStore.showNotification({ text: `Engine renamed to '${renameEngineName.value.trim()}'.`, color: 'success' });
        await engineStore.fetchEngines();
        isRenameDialogOpen.value = false;
    } catch (e) {
        const errorMsg = e.error || e.message || 'Failed to rename engine.';
        console.error('[MyEngines] Error renaming engine:', errorMsg);
        uiStore.showNotification({ text: errorMsg, color: 'error' });
    } finally {
        localIsLoading.value = false;
    }
