...
<script setup>
import { onMounted, computed } from 'vue'; // (PENAMBAHAN KODE) Import 'computed'
import { useNewsStore } from '@/store/news';
import { storeToRefs } from 'pinia';
import NeuralCanvasBackground from '@/components/NeuralCanvasBackground.vue'; // (PENAMBAHAN KODE)
import { format } from 'date-fns';

const newsStore = useNewsStore();
// (PERBAIKAN KODE) Ambil state dan getter baru dari store
const { isLoading, error, paginatedArticles, totalPages } = storeToRefs(newsStore);

// (PENAMBAHAN KODE) Buat computed property untuk v-model pagination
