                    ...
                    <div v-if="activeSection === 'ai_models'">
                         <div class="holo-card pa-8 card-entrance">
                             <div class="d-flex align-center justify-space-between mb-8">
                                <div class="card-label">INFERENCE ENGINE CONFIG</div>
                                <v-btn
                                    variant="text"
                                    color="cyan"
                                    size="small"
                                    ...
                                    class="text-none font-mono"
                                >
                                    RE-SCAN NODES
                                </v-btn>
                             </div>

                             <SettingsField :label="loc('settings_ai_text_model_label')" hint="Default LLM Backend (Path: C:\\FLOWORK\\ai_models\\text)">
                                <v-select
                                ...
                                >
                                     <template v-slot:item="{ props, item }">
                                         <v-list-item v-bind="props" class="model-list-item my-1 mx-2 rounded-0">
                                             <template v-slot:prepend>
                                                 <v-icon :color="item.raw.type === 'local_model' ? 'green-accent-3' : 'purple-accent-2'"
                                                     size="small"
                                                     class="mr-3"
                                                     :icon="item.raw.type === 'local_model' ? 'mdi-server-network' : 'mdi-cloud-outline'"></v-icon>
                                             </template>
                                             <template v-slot:append>
                                                 <span v-if="item.raw.tier" class="text-caption text-gold font-mono border-gold px-1" style="font-size: 0.65rem; border: 1px solid;">{{ item.raw.tier }}</span>
                                             </template>
                                         </v-list-item>
                                     </template>
                                </v-select>
                             ...
                             <v-divider class="my-6 border-cyan-dim"></v-divider>
                             ...
                             <SettingsField :label="loc('settings_ai_gpu_layers_label')" hint="GPU Offload Layers (Local Models)">
                                <v-text-field
                                    ...
                                    color="cyan"
                                    single-line
                                    append-inner-icon="mdi-expansion-card"
                                ></v-text-field>
                             </SettingsField>
                         </div>
                    </div>
