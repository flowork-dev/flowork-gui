<template>
  <div class="settings-layout">
    ...
    <div class="settings-content">
      <v-container fluid>
        <div v-if="isLoading && !localSettings" class="d-flex justify-center align-center fill-height">
          <v-progress-circular indeterminate color="cyan" size="64"></v-progress-circular>
        </div>
        <div v-else-if="error && !localSettings" class="text-center">
        ...
        <div v-else-if="localSettings || activeSection === 'profile'">
          <v-window v-model="activeSection">
            <v-window-item value="profile">
              <ProfileSettings />
            </v-window-item>

            <v-window-item value="gateway_url">
                <h2 class="section-title">{{ loc('settings_section_server') }}</h2>
                <SettingsField :label="loc('settings_server_gateway_url_label')" :hint="loc('settings_server_gateway_url_hint')">
                  <v-text-field
                      v-model="gatewayUrl"
                      variant="outlined"
                      density="compact"
            ...
            <v-window-item value="server_api">
              <h2 class="section-title">{{ loc('settings_section_server') }}</h2>
            ...
            <v-window-item value="cloud_sync">
              <CloudSyncSettings />
            </v-window-item>
          </v-window>
          <v-btn
            v-if="activeSection !== 'profile' && activeSection !== 'cloud_sync' && activeSection !== 'gateway_url'"
            color="cyan"
            @click="handleSave"
            :loading="isLoading"
            class="mt-8 save-button"
...
const gatewayUrl = ref(gatewayStore.gatewayUrl);

const navItems = ref([
  { titleKey: 'Profile', value: 'profile', icon: 'mdi-account-circle-outline' },
  { titleKey: 'settings_server_gateway_url_label', value: 'gateway_url', icon: 'mdi-web-box' },
  { titleKey: 'settings_section_general', value: 'general', icon: 'mdi-tune' },
  { titleKey: 'settings_section_cloud_sync', value: 'cloud_sync', icon: 'mdi-cloud-sync-outline' },
  { titleKey: 'settings_section_ai', value: 'ai_models', icon: 'mdi-brain' },
  { titleKey: 'settings_section_server', value: 'server_api', icon: 'mdi-server-network' },
  { titleKey: 'settings_section_advanced', value: 'advanced', icon: 'mdi-cog-box' },
]);
const aiProviderItems = computed(() => {
  if (!Array.isArray(aiProviders.value) || aiProviders.value.length === 0) {
    return [{ title: 'No models available', value: null, disabled: true }];
  }
...
async function handleSave() {
  if (typeof settings.value !== 'object' || settings.value === null) {
      settings.value = {};
  }
...
async function handleSaveGatewayUrl() {
    gatewayStore.setGatewayUrl(gatewayUrl.value);
    uiStore.showNotification({ text: 'Gateway URL updated. Reloading required to apply changes.', color: 'success' });
}

onMounted(() => {
  settingsStore.fetchSettings();
  aiCenterStore.fetchAiStatus();
  if (!gatewayStore.gatewayUrl) {
      settingsStore.setActiveSection('gateway_url');
  } else {
      settingsStore.setActiveSection('profile');
  }
});
</script>
<style scoped>
...
/* (PENAMBAHAN KODE) Salin style 'hacker-input' dari ProfileSettings */
...
.hacker-input :deep(.v-select__selection-text) {
  color: var(--text-primary) !important;
}
