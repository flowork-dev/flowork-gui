...
# File NAME : C:\FLOWORK\flowork-gui\template\web\src\views\ContactUs.vue
# REVISI:
...
const formStatus = ref('idle'); // English Hardcode
...
const rules = {
  ...
  email: value => /.+@.+\..+/.test(value) || 'E-mail must be valid.', // English Hardcode
};
...
async function handleSubmit() {
  formStatus.value = 'sending'; // English Hardcode
  errorMessage.value = '';
  if (!form.value.name || !form.value.email || !form.value.subject || !form.value.message || !form.value.captcha) {
    errorMessage.value = 'Please fill out all fields.'; // English Hardcode
    formStatus.value = 'error'; // English Hardcode
    return;
  }
  if (parseInt(form.value.captcha, 10) !== (num1.value + num2.value)) {
    errorMessage.value = 'Incorrect captcha answer. Please try again.'; // English Hardcode
    formStatus.value = 'error'; // English Hardcode
    generateCaptcha();
    return;
  }
  console.log('Form data to be sent to cs@teetah.art:'); // English Hardcode
  console.log(JSON.stringify(form.value, null, 2));
  await new Promise(resolve => setTimeout(resolve, 1500));
  formStatus.value = 'success'; // English Hardcode
}
