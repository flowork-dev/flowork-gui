...
// File NAME : C:\FLOWORK\flowork-gui\template\web\src\views\PromptManager.vue total lines 337 
//#######################################################################
    ...
    <v-layout class="fill-height">
      ...
      >
        <v-toolbar color="transparent" density="compact" class="pa-1">
          <v-icon icon="mdi-text-box-multiple" color="cyan-accent-3" class="ml-2"></v-icon>
          <v-toolbar-title class="orbitron-font text-subtitle-1 font-weight-bold ml-2" style="color: #00e5ff;">
            Prompt Templates
          </v-toolbar-title>
          <v-btn icon="mdi-plus" variant="text" color="success" @click="createNewPrompt" title="Create New Prompt"></v-btn>
        </v-toolbar>
        ...
        <v-list nav density="compact" class="prompt-list bg-transparent">
          <div v-if="isLoading" class="text-center py-4">
            <v-progress-circular indeterminate color="cyan-accent-3" size="24"></v-progress-circular>
          ...
          <div v-if="error && !isLoading" class="pa-4 text-center">
             ...
             <v-btn size="x-small" color="cyan-accent-3" variant="text" class="mt-2" @click="promptStore.fetchPrompts()">Retry Connection</v-btn>
          </div>
      ...
      <v-main class="right-panel fill-height bg-grey-darken-4">
        <div v-if="!selectedPrompt" class="empty-state">
          <v-icon icon="mdi-robot-confused-outline" size="80" color="rgba(255,255,255,0.15)"></v-icon>
          <h2 class="mt-4 text-grey-lighten-2 orbitron-font text-h6">Select a prompt or create new</h2>
        ...
        <div v-else class="editor-form pa-4 d-flex flex-column fill-height">
          ...
          <v-card variant="outlined" class="editor-card flex-grow-1 d-flex flex-column mb-4" color="rgba(0, 229, 255, 0.2)">
             <v-toolbar density="compact" color="#161625" class="border-bottom">
                 <span class="text-caption text-cyan-accent-1 ml-4 font-weight-mono">EDITOR MODE</span>
                 <v-spacer></v-spacer>
                 <v-btn size="x-small" variant="text" icon="mdi-content-copy" color="cyan-accent-1" @click="copyToClipboard(selectedPrompt.content)" title="Copy Content"></v-btn>
             </v-toolbar>
             <v-textarea
          ...
          <div class="variable-helper">
              <div class="d-flex align-center mb-2">
                 <v-icon icon="mdi-code-braces" size="small" class="mr-2 text-cyan-accent-3"></v-icon>
                 <span class="text-caption text-white font-weight-bold">Dynamic Variables Injection</span>
              </div>
              <div class="d-flex flex-wrap gap-2">
                  <v-chip @click="copyToClipboard('{{payload.data.my_var}}')" size="small" label color="cyan-accent-3" variant="tonal" class="cursor-pointer font-weight-mono">
                      {{ payloadExampleText }}
                  </v-chip>
                  <v-chip @click="copyToClipboard('{{vars.API_KEY}}')" size="small" label color="purple-accent-2" variant="tonal" class="cursor-pointer font-weight-mono">
                      {{ varsExampleText }}
                  </v-chip>
                  <v-chip @click="copyToClipboard('{{history}}')" size="small" label color="orange-accent-3" variant="tonal" class="cursor-pointer font-weight-mono">
                      {{ historyExampleText }}
                  </v-chip>
              </div>
          </div>
...
async function handleSave() {
    if (selectedPrompt.value) {
        ...
        try {
            ...
            uiStore.showNotification({ text: 'Prompt saved successfully!', color: 'success' });
        } catch (e) {
...
.code-editor :deep(textarea) {
  ...
  font-size: 0.95rem;
  line-height: 1.6;
  color: #00e5ff !important; /* High contrast text color */
...
/* PENTING: Text Colors Override High Contrast */
