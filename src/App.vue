...
# REVISI 8:
# 1. Mengimpor dan menambahkan <GlobalConfirmationDialog />.
# 2. Mengimpor dan menambahkan <GlobalTokenDialog />.
#######################################################################
<template>
  <v-app>
...
const memberAreaPages = [
    'Designer', 'Dashboard', 'MyEngines', 'Settings', 'Profile', // English Hardcode
    'AiCenter', 'AiTrainer', // English Hardcode
    'QuickTools', // English Hardcode
    'News', // English Hardcode
    'MyArticles', 'ArticleEditorNew', 'ArticleEditorEdit', // English Hardcode
    'ModelFactory', 'ModuleFactory', 'PromptManager', 'CoreEditor', 'Diagnostics', // English Hardcode
    'AuthorizeEngine' // English Hardcode
];
const isMemberAreaPage = computed(() => memberAreaPages.includes(route.name));

const publicPagesWithLanderHeader = [
    'Lander', 'PrivacyPolicy', 'TermsOfService', 'DMCA', 'ContactUs', // English Hardcode
    'Login', 'Register', 'ArticleView', 'AuthorView', 'CategoryView', 'TagView', 'ArticleListEnglish', 'ArticleListIndonesian' // English Hardcode
];
const isPublicPageWithLanderHeader = computed(() => publicPagesWithLanderHeader.includes(route.name));
...
watch(() => authStore.isAuthenticated, (isAuth) => {
  if (isAuth) {
    if (!socketStore.isConnected || !socketStore.isAuthenticated) {
        socketStore.connect();
    }
  } else {
