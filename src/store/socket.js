...
// File NAME : C:\FLOWORK\flowork-gui\template\web\src\store\socket.js total lines 651 
//#######################################################################
        ...
        socket.value.on('WORKFLOW_LOG_ENTRY', (data) => {
            ...
            if (payload.node_id) {
                ...
                workflowStore.updateNodeExecutionStatus({
                    ...
                    workflow_context_id: payload.job_id // Helps store filter if needed
                });
                ...
                logStore.updateTimelineEntry({
                    node_id: payload.node_id,
                    module_id: payload.source, // Payload has 'source', node might not be loaded yet
                    status: status,
                    timestamp: payload.ts ? new Date(payload.ts).getTime() / 1000 : Date.now() / 1000,
