...
// [FLOWORK FIX] Import 'api' (apiClient) dengan benar
import api, {
    apiFetchEngines,
    apiRegisterEngine,
    apiDeleteEngine,
...
export const useEngineStore = defineStore('engines', () => {
    ...
    // [FLOWORK FIX] Action Execute Module via RAW WORKFLOW
    // Ini nembak route 'POST /api/v1/workflow/execute_raw' yang pasti ada di backend
    async function runModule(engineId, moduleId, payload) {
        ...
        try {
            ...
            // Kita bungkus Module jadi Workflow Single Node
            const rawWorkflowBody = {
                nodes: [
                    {
                        id: "direct_exec_node",
                        type: moduleId, // ID Module (contoh: prompt_receiver_module)
                        config: {},     // Config default
                        position: { x: 0, y: 0 }
                    }
                ],
                connections: [],
                initial_payload: payload, // Payload user masuk sini
                start_node_id: "direct_exec_node",
                mode: "EXECUTE"
            };

            // Tembak Endpoint Resmi
