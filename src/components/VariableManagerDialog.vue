...
// File NAME : C:\FLOWORK\flowork-gui\template\web\src\components\VariableManagerDialog.vue
//#######################################################################
...
// LOGIC UNTUK MEMBERSIHKAN DATA (UNWRAP)
watch(variables, (newVars) => {
    localVariables.value = (newVars || []).map(v => {
        let displayVal = '';
        let rawVal = v.value;

        // 1. Cek apakah ini object JSON yang membungkus value?
        // Contoh: { "type": "static", "value": "CHANGE THIS API" }
        if (typeof rawVal === 'object' && rawVal !== null && !Array.isArray(rawVal) && 'value' in rawVal) {
             rawVal = rawVal.value; // Ambil isinya aja
        }

        // 2. Konversi ke String buat ditampilkan di Textarea
        if (Array.isArray(rawVal)) {
            displayVal = rawVal.join('\n');
        } else if (typeof rawVal === 'object' && rawVal !== null) {
            // Fallback kalau masih object tapi gak punya properti 'value'
            try {
    ...
    try {
        ...
        for (const item of localVariables.value) {
            if (item.name) {
                ...
                // Logic: Textarea is the source of truth
                if (item.mode === 'random' || item.mode === 'sequential') {
                    // Split baris jadi array
                    ...
                    // Single mode: Kirim string murni
                ...
                // Bersihkan field temporary sebelum kirim ke backend
