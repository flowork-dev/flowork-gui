...
# File NAME : C:\FLOWORK\flowork-gui\template\web\src\components\ArticleComments.vue
# PERBAIKAN: Mengganti 'authStore.user.public_address'
#            menjadi 'authStore.user.id' di 'isAuthor' dan 'isModerator'.
#######################################################################
<template>
  ...
  try {
    ...
    if (result.error) {
        if (result.status === 429) {
            rateLimitError.value = result.error || 'Rate limit exceeded.'; // English Error
            uiStore.showNotification({ text: rateLimitError.value, color: 'error' });
        } else {
            postError.value = result.error || `HTTP error ${result.status}`; // English Error
    ...
    if (!postError.value && !rateLimitError.value) {
        const errorMsg = err.message || "Could not post comment."; // English Error
...
async function saveEdit(comment) {
  ...
  try {
    ...
    uiStore.showNotification({ text: 'Comment updated!', color: 'success' }); // English Hardcode
    cancelEdit();
  } catch (err) {
    console.error("Failed to update comment:", err);
    uiStore.showNotification({ text: `Update failed: ${err.message}`, color: 'error' }); // English Hardcode
  } finally {
...
// --- (PERBAIKAN KUNCI) ---
// Mengganti authStore.user.public_address menjadi authStore.user.id
