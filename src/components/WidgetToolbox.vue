...
// File NAME : C:\FLOWORK\flowork-gui\template\web\src\components\WidgetToolbox.vue total lines 205 
//#######################################################################
  ...
  <div class="widget-toolbox-container">
    <div class="toolbox-header">
      <div class="title-row">
        ...
        <div class="badge-counter" v-if="widgets.length > 0">{{ widgets.length }}</div>
      </div>
      <button class="upload-btn" @click="triggerUpload" title="Upload Widget Folder">
    ...
    <div v-else-if="filteredWidgets.length === 0 && searchQuery" class="state-container empty">
       <p>No apps found for "{{ searchQuery }}"</p>
    </div>

    <div v-else-if="widgets.length === 0" class="state-container empty">
    ...
    <div v-else class="widget-grid-wrapper">
      <div class="widget-grid">
        ...
        <div v-for="widget in filteredWidgets" :key="widget.id" class="widget-item" @click="handleWidgetClick(widget.id)" v-tooltip.top="widget.description || widget.name">
          <div class="icon-wrapper glass">
            <svg style="width:36px;height:36px;fill:#00FFFF;filter:drop-shadow(0 0 5px rgba(0,255,255,0.5));" viewBox="0 0 24 24">
                <path d="M21,16.5C21,16.88 20.79,17.21 20.47,17.38L12.57,21.82C12.41,21.94 12.21,22 12,22C11.79,22 11.59,21.94 11.43,21.82L3.53,17.38C3.21,17.21 3,16.88 3,16.5V7.5C3,7.12 3.21,6.79 3.53,6.62L11.43,2.18C11.59,2.06 11.79,2 12,2C12.21,2 12.41,2.06 12.57,2.18L20.47,6.62C20.79,6.79 21,7.12 21,7.5V16.5M12,4.15L6.04,7.5L12,10.85L17.96,7.5L12,4.15M5,15.91L11,19.29V12.58L5,9.21V15.91M19,15.91V9.21L13,12.58V19.29L19,15.91Z" />
            </svg>
          </div>
...
const widgets = computed(() => widgetStore.installedWidgets);
...
onMounted(() => { widgetStore.fetchInstalledWidgets(); });
const handleWidgetClick = (id) => { widgetStore.openWidget(id); };
const triggerUpload = () => { if (folderInput.value) folderInput.value.click(); };

const handleFolderSelect = async (event) => {
    const files = event.target.files;
    if (!files || files.length === 0) return;
    console.log(`[WidgetToolbox] Processing ${files.length} files...`);
    const packageData = { manifest: {}, files: {}, type: 'widget' };
...
<style scoped>
.widget-toolbox-container { height: 100%; width: 280px; display: flex; flex-direction: column; padding: 1.5rem; color: var(--text-color); font-family: 'Inter', sans-serif; }
/* [MODIFIED] Header padding dikurangin dikit biar muat search bar */
...
.search-input {
  ...
  color: #fff;
  padding: 8px 30px 8px 32px; /* Kiri buat icon, kanan buat clear */
  border-radius: 8px;
  font-size: 0.85rem;
  outline: none;
  transition: all 0.2s;
...
.search-icon {
  position: absolute; left: 8px; width: 16px; height: 16px; fill: rgba(255, 255, 255, 0.4); pointer-events: none;
}
.clear-search {
  position: absolute; right: 10px; color: rgba(255, 255, 255, 0.5); cursor: pointer; font-weight: bold; font-size: 1.1rem;
}
...
.widget-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1.25rem 0.5rem; }
.widget-item { display: flex; flex-direction: column; align-items: center; cursor: pointer; position: relative; }
.icon-wrapper { width: 56px; height: 56px; border-radius: 16px; display: flex; align-items: center; justify-content: center; margin-bottom: 0.5rem; transition: all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1); position: relative; overflow: hidden; }
...
.widget-name {
  ...
  line-height: 1.2;
  max-width: 100%;
  /* [UPDATED] Line Clamp Standard for Future-Proofing */
  display: -webkit-box;
  -webkit-line-clamp: 2;
  line-clamp: 2;
