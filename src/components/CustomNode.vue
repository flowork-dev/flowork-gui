...
// COMMENT: [PENGURANGAN] useVueFlow dihapus dari sini karena ini sumber errornya.
import { Handle, Position } from '@vue-flow/core';
import { useWorkflowStore } from '@/store/workflow';
// (PENAMBAHAN KODE) Import component store untuk mencari manifest
...
const props = defineProps({
  ...
  // COMMENT: [PENAMBAHAN] Kita terima status zoom dari parent (Designer.vue)
  isZoomedOut: { type: Boolean, default: false },
});

const workflowStore = useWorkflowStore();
const localeStore = useLocaleStore();
// (PENAMBAHAN KODE) Inisialisasi component store
...
// (PERBAIKAN KUNCI) MANIFEST SEKARANG DIAMBIL DARI COMPONENT STORE, BUKAN DARI PROPS
...
// (PERBAIKAN KUNCI) COMPONENT TYPE JUGA DIAMBIL DARI PROPS.DATA
...
const nodeStyle = computed(() => {
  // (PERBAIKAN) Ambil warna dari manifest jika di data node tidak ada
...
<style scoped>
/* (COMMENT) ... style tidak ada perubahan ... */
.node-lod {
  width: 24px;
  height: 24px;
  border-radius: 4px;
...
.vue-flow__node-default {
  ...
  padding: 0 !important;
  width: auto !important; /* Biar ukurannya pas dengan konten kita */
