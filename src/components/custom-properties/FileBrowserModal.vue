...
const props = defineProps({
  ...
  // TAMBAHAN: Mode seleksi ('folder' atau 'file'). Default 'folder' biar logic lama aman.
...
// TAMBAHAN: State buat nyimpen file yang diklik user
...
function fetchFiles(path) {
  ...
  error.value = null;
  items.value = [];
  // Reset pilihan file kalau pindah folder
...
function handleItemClick(item) {
  if (item.is_dir) {
    ...
    if (item.type === 'drive') {
      ...
      // LOGIC BARU: Handle Klik File (hanya kalau mode file)
...
// Helper buat styling item yang dipilih
function isSelected(item) {
    if (props.selectionMode !== 'file') return false;
    if (item.is_dir) return false;

    // Cek apakah item ini adalah selectedFilePath
...
function confirmSelection() {
  // LOGIC BARU: Return file path kalau mode file, folder path kalau mode folder
  if (props.selectionMode === 'file') {
...
watch(() => props.modelValue, (val) => {
    if (val) {
        ...
        // Kalau ada initialPath berupa file, coba buka foldernya (opsional, logic simpel dulu)
        fetchFiles(DEFAULT_PATH);
    }
});
</script>
