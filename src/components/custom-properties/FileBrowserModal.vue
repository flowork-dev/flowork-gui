...
// (PERBAIKAN KODE) Import socket store
...
// (PENAMBAHAN KODE) Inisialisasi store
...
const handleDrivesResponse = (data) => {
    ...
    if (data.error) {
        ...
        currentPath.value = ''; // Kosongkan path saat di root/drives
        isInitialView.value = true;
    }
};

const handleDirectoryResponse = (data) => {
    // Hanya update jika path response sesuai dengan path yang diminta terakhir
...
onMounted(() => {
    // Dengarkan event dari socket store
...
async function fetchInitialView() {
    ...
    try {
        // (PERBAIKAN KODE) Kirim request via WebSocket
    ...
    // isLoading akan di-set false oleh handleDrivesResponse
}
...
async function fetchItems(path) {
  ...
  // isLoading akan di-set false oleh handleDirectoryResponse
}
...
function handleItemClick(item) {
    ...
    if (item.type === 'directory' || item.type === 'drive') {
        ...
        // Jika file diklik, mungkin pilih file tersebut? Untuk sekarang abaikan.
...
function goUp() {
    ...
    // (PERBAIKAN KODE) Logika go up disederhanakan dan lebih robust
...
function selectCurrentFolder() {
  // Hanya emit jika bukan tampilan drive awal dan path valid
...
const getIcon = (item) => {
    ...
    // Icon file berdasarkan ekstensi (contoh sederhana)
