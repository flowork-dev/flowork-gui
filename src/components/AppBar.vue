...
// File NAME : C:\FLOWORK\flowork-gui\template\web\src\components\AppBar.vue total lines 423 
//#######################################################################
...
<template>
  <v-app-bar flat class="app-bar" app>
    ...
    <template v-if="!authStore.isAuthenticated">
        <v-btn :to="{ name: 'Marketplace' }" variant="text" class="mx-2 orbitron-font" color="grey-lighten-1">Marketplace</v-btn>
        <v-btn :to="{ name: 'News' }" variant="text" class="mx-2 orbitron-font" color="grey-lighten-1">News</v-btn>
    </template>
    <v-btn v-if="!authStore.isAuthenticated" to="/login" variant="tonal" class="mx-2 orbitron-font" color="cyan" prepend-icon="mdi-login-variant">Login</v-btn>
...
const isMenuOpen = ref(false); // [FIX] Controlled state
...
const mainNavGroups = ref([
  {
    ...
    items: [
      ...
      { title: 'Widgets', icon: 'mdi-view-grid-plus-outline', to: { name: 'Widgets' } }, // <--- INI DIA!
