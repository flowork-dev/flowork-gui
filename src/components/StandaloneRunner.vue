...
// Inisialisasi configValues saat komponen dibuka
watch(() => props.componentData, (newComponent) => {
  if (newComponent) {
    // Reset state saat dialog baru dibuka
    logStore.clearLogs();
    activeTab.value = 'config';
    jobStatus.value = null;
    currentJobId.value = null;

    // Set config_values dari default manifest
    const defaults = {};
...
// Nonton status workflow dari store
watch(() => logStore.executionLogs, (logs) => {
    if (!currentJobId.value) return;

    // Cari log status terakhir untuk job ini
