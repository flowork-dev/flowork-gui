...
// File NAME : C:\FLOWORK\flowork-gui\template\web\src\components\ControlsFooter.vue total lines 932 
//#######################################################################
...
<template>
  <div class="footer-area-wrapper">
    ...
    >
      <v-sheet class="mini-prompt-sheet" elevation="10" rounded="xl">
        <v-text-field
          v-model="quickPrompt"
          placeholder="Send instant prompt to Agent..."
          variant="solo"
          density="compact"
          hide-details
          bg-color="rgba(20, 20, 20, 0.95)"
          class="mini-prompt-input"
          @keyup.enter="sendQuickPrompt"
...
async function sendQuickPrompt() {
    ...
    try {
        ...
        uiStore.showNotification({ text: "Prompt sent!", color: "success" });
        quickPrompt.value = "";
    } catch (e) {
