  ...
  <v-list density="compact" class="component-list">
    ...
    <div v-else>
      <div v-if="filteredFavorites.length > 0">
        ...
        >
          <template v-slot:activator="{ props: tooltipProps }"> <v-list-item
            ...
            >
              <template v-slot:prepend>
                <v-img
                  v-if="item.manifest.icon_file"
                  :src="getComponentIconUrl(type, item.id)"
                  class="component-icon"
                  width="24"
                  height="24"
                  cover
                ></v-img>
                <v-icon v-else class="component-icon" :icon="defaultIcons[type]"></v-icon>
              </template>
              <div>
                <v-list-item-title class="item-title">{{ loc(item.name) }}</v-list-item-title>
          ...
          <div class="ai-tooltip-content">
            <div class="font-weight-bold mb-1">{{ loc(item.name) }}</div>
            {{ loc(item.manifest.description) || 'No description available.' }}
...
const defaultIcons = {
    modules: 'mdi-cog-outline',
    plugins: 'mdi-power-plug-outline',
    tools: 'mdi-hammer-wrench',
    triggers: 'mdi-timer-sand-outline'
};
...
const onDragStart = (event, item) => {
  if (event.dataTransfer) {
    if (!item.is_installed) {
        ...
        console.warn(`[ComponentList] Drag aborted: Component ${item.id} is not installed.`); // English Log
        return;
    }
...
const onItemClick = (item) => {
  console.log(`[ComponentList] Item clicked: ${item.id}`); // English Log
...
const baseFilteredItems = computed(() => {
    ...
    if (props.searchTerm) {
        ...
        items = state.value.items.filter(item =>
          loc(item.name).toLowerCase().includes(term) ||
          item.id.toLowerCase().includes(term) ||
          (loc(item.manifest?.description) || '').toLowerCase().includes(term) // (PERBAIKAN) Tambahkan fallback string kosong
        );
    }
    return items;
});
...
onMounted(() => {
  observer = new IntersectionObserver((entries) => {
    if (entries[0].isIntersecting) {
      // (COMMENT) This handles the initial load for the first tab
      // (PERBAIKAN) Panggil tanpa reset, biarkan 'watch' yang handle reset
...
// (PENAMBAHAN KODE) This watch correctly loads new data WHEN THE TAB CHANGES (e.g., Modules -> Plugins).
// The initial load for the first tab is handled by the IntersectionObserver in onMounted.
watch(() => props.type, (newType, oldType) => {
    if (newType !== oldType) {
        console.log(`[ComponentList] Type changed to ${newType}. Fetching components with reset...`); // English Log
        // (PERBAIKAN) Gunakan reset: true agar data tab baru dimuat ulang
...
/* --- KUNCI PERBAIKAN: Tooltip Style --- */
.ai-tooltip-content {
  /* Set lebar maksimal untuk konten pop-up */
  max-width: 300px; /* English Hardcode */
  white-space: normal; /* Penting agar teks wrap */
  word-wrap: break-word;
  padding: 12px;
}
/* Memastikan container tooltip Vuetify juga merespons */
:deep(.ai-tooltip) {
    max-width: 320px !important; /* English Hardcode */
    pointer-events: none;
}
/* --- AKHIR PERBAIKAN Tooltip --- */


/* --- (PEROMBAKAN STYLE TOMBOL) --- */
.button-group {
    opacity: 0; /* Sembunyikan semua tombol by default */
    transition: opacity 0.2s ease;
    display: flex;
    align-items: center;
}
/* Tampilkan grup tombol saat item di-hover */
.toolbox-item:hover .button-group,
/* Tampilkan grup tombol jika komponen sedang loading (tombol disabled) */
.toolbox-item .button-group .v-btn[disabled],
/* (PERBAIKAN) Tampilkan grup tombol jika tombol favorit aktif (bintang penuh) */
.toolbox-item .button-group .favorite-btn .v-icon.mdi-star {
    opacity: 1;
}

/* (PENAMBAHAN KODE) Style khusus untuk tombol install/uninstall */
.install-btn {
    width: 24px; /* Samakan lebar dengan tombol favorit */
}
/* --- (AKHIR PEROMBAKAN STYLE) --- */
</style>