...
// File NAME : C:\FLOWORK\flowork-gui\template\web\src\components\Toolbox.vue total lines 398 
//#######################################################################
...
const targetItemForUpload = ref(null); // (English Hardcode) To verify selected folder matches item
...
const tabs = ref([
    ...
    { titleKey: 'toolbox_tab_tools', value: 'tools', icon: 'mdi-hammer-wrench' },
    { titleKey: 'toolbox_tab_triggers', value: 'triggers', icon: 'mdi-timer-sand-outline' }
]);
...
watch(() => engineStore.selectedEngineId, (newId) => {
    if (newId) {
        componentStore.forceRefetchAllComponents();
    }
});
