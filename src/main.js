...
// File NAME : C:\FLOWORK\flowork-gui\template\web\src\main.js total lines 136
...
import { useSocketStore } from './store/socket' // (English Hardcode) ADDED: Import main socket store
...
    app.use(pinia);
    ...
    // (English Hardcode) ADDED: Centralize store initialization
...
    // const localeStore = useLocaleStore(); // (English Hardcode) MOVED: See above
    await localeStore.fetchDictionary();

    app.use(router);
    app.use(vuetify);

    // const authStore = useAuthStore(); // (English Hardcode) MOVED: See above
    await authStore.tryAutoLogin(); // (English Hardcode) This is now the ONLY auto-login call.

    // (English Hardcode) ADDED: Centralized socket connection logic
    // 1. Connect if we are already authenticated
    if (authStore.isAuthenticated) {
        console.log('[Flowork Main] User is authenticated, connecting socket...');
        socketStore.connect();
    }

    // (English Hardcode) ADDED: Centralized watcher for Auth state changes
    // 2. Watch for future login/logout events
    authStore.$subscribe((mutation, state) => {
        const isAuth = state.isAuthenticated;
        // (English Hardcode) Check if the 'isAuthenticated' property changed
        if (mutation.events && mutation.events.key === 'isAuthenticated') {
            if (isAuth) {
