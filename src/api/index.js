...
const DEFAULT_API_TIMEOUT = 20000; // [FIX AWENK] Naikkan timeout jadi 20s biar aman
 ...
 * Added explicit check for 'flowork_gateway_token'.
 */
export const getAuthHeaders = async (fullUrl, method = 'GET') => {
    ...
    // 1. [FIX AWENK] Cek Token Standar & Gateway Token
    // Kita prioritasin token yang ada di store, tapi backup ke localStorage
    let token = authStore.token || localStorage.getItem('flowork_gateway_token') || localStorage.getItem('token');

    if (token) {
        headers['Authorization'] = `Bearer ${token}`;
        headers['x-gateway-token'] = token; // [FIX] Header wajib buat Gateway
    }

    // 2. Web3 / Private Key Signing Logic (ORIGINAL CODE RESTORED)
...
// [AWENK] Ini interceptor sakti yang manggil getAuthHeaders di atas
const cryptoInterceptor = async config => {
    ...
    // Inject headers including the Gateway Token
...
// [AWENK] Export default wajib ada biar import di store lain (misal import api from '@/api') tetep jalan
export { apiClient };
