...
# File NAME : database_service.py (SELF HEALING EDITION)
########################################################################
...
class DatabaseService(BaseService):
    ...
    def _check_and_heal_table(self, cursor, table_name, required_column):
        """
        [SELF-HEAL] Cek apakah tabel cacat? Jika ya, hapus biar dibikin ulang.
        """
        try:
            cursor.execute(f"PRAGMA table_info({table_name})")
            columns = [info[1] for info in cursor.fetchall()]

            # Kalau tabel ada TAPI kolom wajib gak ada -> CACAT!
            if columns and required_column not in columns:
                self.logger.critical(f"[SELF-HEAL] 🚑 Table '{table_name}' corrupt! Missing '{required_column}'. DROPPING TABLE...")
                cursor.execute(f"DROP TABLE IF EXISTS {table_name}")
                self.logger.info(f"[SELF-HEAL] ✅ Table '{table_name}' dropped. Will be recreated.")
                return True # Ada perbaikan
        except Exception as e:
            self.logger.warning(f"[SELF-HEAL] Check failed for {table_name}: {e}")
        return False
    ...
    def create_tables(self):
        ...
        try:
            ...
            # --- [FASE 0: SELF HEALING CHECKS] ---
            # Cek tabel Jobs, harus punya job_id
            self._check_and_heal_table(cursor, "Jobs", "job_id")
            # Cek tabel Executions, harus punya execution_id
            self._check_and_heal_table(cursor, "Executions", "execution_id")
            # -------------------------------------
